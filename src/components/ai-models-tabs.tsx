...
// Updated tab data with description property
const tabData = [
	{
		id: "market",
		...
		<section className="py-10 px-4 bg-gray-50">
			<div className="container mx-auto max-w-6xl">
				...
				<div
					...
					className="md:hidden flex overflow-x-auto space-x-2 pb-3 hide-scrollbar"
				>
					{tabData.map((tab) => (
						<button
							key={tab.id}
				...
				<div className="md:hidden flex justify-between items-center mt-2 mb-4">
					...
					>
						<ChevronLeft
							size={20}
							className={activeTab === tabData[0].id ? "opacity-50" : ""}
						/>
					</button>
					<div className="text-sm font-medium text-gray-500">
						{tabData.findIndex((tab) => tab.id === activeTab) + 1} of{" "}
						{tabData.length}
					</div>
					<button
					...
					>
						<ChevronRight
							size={20}
							className={
								activeTab === tabData[tabData.length - 1].id ? "opacity-50" : ""
							}
						/>
					</button>
				</div>
